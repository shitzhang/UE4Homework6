...
void AShooterBaseCharacter::BeginPlay()
{
	...
	HealthComp->OnHealthChanged.AddDynamic(this, &AShooterBaseCharacter::OnHealthChanged);
}
...
{
	Die(HealthComp->GetHealth(), FDamageEvent(DmgType.GetClass()), NULL, NULL);
}
...
void AShooterBaseCharacter::OnHealthChanged(UShooterHealthComponent* OwningHealthComp, float Health, float Damage,
	const class UDamageType* DamageType, class AController* InstigatedBy, AActor* DamageCauser)
{
	...
	//		if (DamageType)
	//		{
	//			const UShooterDamageType* DmgType = Cast<UShooterDamageType>(DamageType);
	//			bCanDie = (DmgType == nullptr || (DmgType && DmgType->GetCanDieFrom()));
	//		}
	...
	//		{
	//			/* Player cannot die from this damage type, set hitpoints to 1.0 */
	//			OwningHealthComp->Health = 1.0f;
	//		}
	//	}
	//	else
	//	{
	//		APawn* Pawn = InstigatedBy ? InstigatedBy->GetPawn() : nullptr;
	//		PlayHit(Damage, DamageType, Pawn, DamageCauser, false);
	//	}
	//}
